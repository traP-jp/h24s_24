    ...
    <div class="post-view-container">
      <div v-if="postContent != undefined">
        <div v-for="ancestor in postContent.ancestors" :key="ancestor.post.id">
          <Post :content="ancestor.post.converted_message" :date="new Date(ancestor.post.created_at)"
        ...
        <hr />
        <Post :content="postContent.converted_message" :date="new Date(postContent.created_at)"
          :name="postContent.user_name" :reactions="convertReactions(postContent.reactions, postContent.my_reactions)"
          :id="postContent.id" @react="loadPost" />
        <hr />
        <NewPostSection name="" :parent-id="postContent.id" @submit="loadPost" />
        <!-- TODO: -->
        <div v-for="child in postContent.children" :key="child.post.id">
          <Post :content="child.post.converted_message" :date="new Date(child.post.created_at)"
            :name="child.post.user_name" :reactions="convertReactions(child.post.reactions, child.post.my_reactions)"
            :id="child.post.id" @react="loadPost" />
        </div>
      </div>
    </div>
  </MainLayout>
